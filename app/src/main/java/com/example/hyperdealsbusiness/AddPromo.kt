    ...
    fun setupPlacesAutocomplete(){
        ...
        autocompleteFragment.setOnPlaceSelectedListener(object: PlaceSelectionListener {
            override fun onPlaceSelected(p0: Place) {
                val getlat= p0.latLng!!.latitude.toString()
                val getlong = p0.latLng!!.longitude.toString()
        ...
        if (imageUri!=null){
            ...
            ref.putFile(imageUri!!)
                .addOnSuccessListener {
                    ...
                    val image = ref.downloadUrl.toString()
...
//                Log.e(TAG," selected ${ LoginActivityBusinessman.globalUserBusinessman.stores[position]}")
    ...
    fun getStores2() {
        ...
        doAsync {
            database.collection("UserBusinessman").document(LoginActivityBusinessman.userBusinessManUsername).collection("Stores").get().addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    for (DocumentSnapshot in task.result!!) {
